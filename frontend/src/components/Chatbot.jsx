...
export default function Chatbot({ results, isOpen = true, onClose, mode = 'overlay', groqApiKey = '' }) {
  ...
  if (isPage) {
    return (
      <div className="w-full h-full bg-surface-base flex flex-col overflow-hidden animate-fade-up">
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          {messages.map((msg, i) => (
            <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[80%] rounded-xl px-4 py-3 text-sm leading-relaxed shadow-sm ${
                msg.role === 'user'
  ...
  return (
    <div className="fixed inset-0 z-[999] flex animate-fade-up">
      <div className="absolute inset-0 bg-black/40 backdrop-blur-[2px]" onClick={onClose} />
      <div className="relative w-full h-full bg-surface-base border border-border-subtle flex flex-col overflow-hidden shadow-2xl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-border-subtle bg-surface-raised/50 shrink-0">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center">
              <Bot className="w-4 h-4 text-primary" />
